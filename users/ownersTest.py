...
def test_listar_propietario():
    try:
        ...
        response = requests.get(f"{URL_BASE}/homeowners", headers=headers, params=LISTADO)
        response.raise_for_status()
        datos = response.json()
...
def crear_propietario():
    ...
    try:
        headers = {"Authorization": f"Bearer {token}"}
        response = requests.post(f"{URL_BASE}/homeowners", json=PROPIETARIO_A_CREAR, headers=headers)
        response.raise_for_status()
        datos = response.json()
...
def test_llamar_propietario(crear_propietario):
    try:
        ...
        headers = {"Authorization": f"Bearer {token}"}
        response = requests.get(f"{URL_BASE}/homeowners", headers=headers, params=params)
        response.raise_for_status()
        datos = response.json()
...
def test_actualizar_propietario(crear_propietario):
    try:
        token = obtenerToken()
        headers = {"Authorization": f"Bearer {token}"}
        response = requests.put(f"{URL_BASE}/homeowners/{crear_propietario}", json=PROPIETARIO_A_ACTUALIZAR, headers=headers)
...
def test_eliminar_propietario(crear_propietario):
    try:
        token = obtenerToken()
        headers = {"Authorization": f"Bearer {token}"}
        response = requests.delete(f"{URL_BASE}/homeowners/{crear_propietario}", headers=headers)
        response.raise_for_status()
        datos = response.json()
